...
class TestResumeUpdate:
    """Tests for resume update."""

    async def test_update_resume_title(self, client: AsyncClient, auth_headers):
    ...
    async def test_update_resume_content(self, client: AsyncClient, auth_headers):
        ...
        # Update content
        new_content = "Updated resume content"
        response = await client.patch(
            f"/v1/resumes/{resume_id}",
            headers=auth_headers,
    ...
    async def test_update_resume_not_found(self, client: AsyncClient, auth_headers):
...
class TestResumeDelete:
    ...
    async def test_delete_resume(self, client: AsyncClient, auth_headers):
        ...
        response = await client.delete(
            f"/v1/resumes/{resume_id}", headers=auth_headers
        )
    ...
    async def test_delete_resume_not_found(self, client: AsyncClient, auth_headers):
        """Test deleting non-existent resume."""
        response = await client.delete("/v1/resumes/99999", headers=auth_headers)
        assert response.status_code == 404
