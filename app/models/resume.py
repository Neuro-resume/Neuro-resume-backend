...
class Resume(Base):
    ...
    template = Column(
        Enum(ResumeTemplate, name="resumetemplate", native_enum=True, create_constraint=True),
        default=ResumeTemplate.MODERN,
        nullable=False
    )
    language = Column(
        Enum(ResumeLanguage, name="resumelanguage", native_enum=True, create_constraint=True),
...
class LinkInfo(BaseModel):
    ...
    type: str = Field(..., description="Link type: linkedin, github, telegram, website, portfolio")
    url: HttpUrl
...
class WorkExperience(BaseModel):
    ...
    end_date: Optional[str] = Field(None, description="YYYY-MM format or null if current")
...
class ResumeCreateSimple(BaseModel):
    ...
    format: Optional[str] = Field(None, description="Format for compatibility (pdf, docx, txt)")
...
class ResumeResponse(ResumeBase):
    ...
    format: Optional[str] = Field(None, description="Format (derived from template)")
    content: Optional[Dict[str, Any]] = Field(None, description="Content (alias for data)")
    ...
    def model_post_init(self, __context) -> None:
        ...
        if self.content is None:
            self.content = self.data.model_dump() if self.data else {}
